...
namespace Entity
{
    public class LootHolderSubsystem : BaseSubsystem
    {
        [Header("Loot Table")]
        ...
        private void RollAndSpawnLoot()
        {
            foreach (var entry in lootTable.entries)
            {
                ...
                for (int i = 0; i < amount; i++)
                    SpawnCollectable(entry.resource);
            }
        }
        
        private void SpawnCollectable(ResourceSO resource)
        {
            ...
            BaseCollectable spawned = Instantiate(resource.worldPrefab, start, Quaternion.identity);
            if (!spawned)
                return;
